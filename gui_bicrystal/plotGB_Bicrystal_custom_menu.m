...
uimenu(menuBX, 'Label', 'Save bicrystal pictures', ...
    ...
    'save_figure(gui.GB.pathnameRB_BC, gcf)'], 'Separator','on');
...
uimenu(menuBX, 'Label', 'Edit YAML bicrystal config. file', ...
    'Callback', ['gui = guidata(gcf);' ...
    'if gui.GB.YAMLfilename edit(gui.GB.YAMLfilename); end'], ...
    'Separator','on');
...
uimenu(menuBX_CPFEM, 'Label', 'Bending test of a cantilever', ...
    'Callback', ['commandwindow; beep;' ...
    'warning(''Bending test model not yet implemented...!'')']);
uimenu(menuBX_CPFEM, 'Label', 'Compression test of µ-pillar', ...
    'Callback', ['commandwindow; beep;' ...
    'warning(''Compression of µ-pillar model not yet implemented...!'')']);
