...
for ig = grain
    ...
    if flag_error == 0
        ss = slip_systems(phase, listslip);
        if strcmp(phase, 'hcp') == 1
            for ss_ind = 1:size(ss, 3)
                ss_cart(1,:,ss_ind) = millerbravaisplane2cart(ss(1,:,ss_ind), lattice_parameters(1));
                ss_cart(2,:,ss_ind) = millerbravaisdir2cart(ss(2,:,ss_ind), lattice_parameters(1));
...
if flag_error == 0
    if lattice_parameters(1) ~= 0
        % Preallocation
        sortbv   = NaN(size(ss, 3), 15, grain);
        slip_vec = NaN(size(ss, 3), 15);
        g_mat      = NaN(3, 3, grain);
        vect = NaN(size(ss, 3), 16, grain);
