...
msc_module_path = strrep(gui_SX.config_CPFEM.msc_module_path, '\', '\\');
...
py{end+1} = 'import sys';
py{end+1} = sprintf('p=''%s''', msc_module_path);
py{end+1} = sprintf('if p not in sys.path: sys.path.insert(0,p) \n');
py{end+1} = sprintf('import proc');
py{end+1} = sprintf('from proc.bicrystal import Indentation');
...
py{end+1} = sprintf('Indentation.MENTATVERSION = %.1f', gui_SX.config_CPFEM.fem_solver_version); 
py{end+1} = sprintf('Indentation.MENTATVERSION_STR = ''%s''', gui_SX.config_CPFEM.fem_solver_used); 
