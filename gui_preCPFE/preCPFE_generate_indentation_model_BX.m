...
[gui_BX.GB.Titlegbdata, gui_BX.GB.Titlegbdatacompl] = preCPFE_set_title_data(gui_BX.config_map, gui_BX.GB);
...
% fprintf(fid, '# Generated by matlab/gui/preCPFE_generate_model.m --- DO NOT EDIT\n');
% fprintf(fid, 'import os,sys\n');
% fprintf(fid, 'p=''%s'' \n', gui_BX.config.CPFEM.msc_module_path);
...
% fprintf(fid, 'gb_ind.doit(gb_data, proc_path=r''%s'')\n', fullfile(gui_BX.config.CPFEM.proc_file_path, gui_BX.GB.Titlegbdata, ''));
% fclose(fid);

python4fem_module_path = strrep(gui_BX.config.CPFEM.python4fem_module_path, '\', '\\');
...
% proc_path_inp file for AFM topography
...
py{end+1} = sprintf('BicrystalIndent.CODE = ''%s''', gui_BX.config.CPFEM.simulation_code);
py{end+1} = sprintf('BicrystalIndent.FEMSOFTWAREVERSION = %.1f', gui_BX.config.CPFEM.fem_solver_version);
py{end+1} = sprintf('BicrystalIndent.FEMSOFTWARE = ''%s''', strtok(gui_BX.config.CPFEM.fem_solver_used, '_'));
...
py{end+1} = sprintf('geo = ''%s'',', gui_BX.indenter_type);
py{end+1} = sprintf('free_mesh_inp = ''%s''', strcat(gui_BX.GB.Titlegbdata, '.inp'));
py{end+1} = ')';
