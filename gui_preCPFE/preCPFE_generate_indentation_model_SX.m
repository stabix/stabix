...
[gui_SX.GB.Titlegbdata, gui_SX.GB.Titlegbdatacompl] = preCPFE_set_title_data(gui_SX.config_map, gui_SX.GB);
...
% fprintf(fid, 'p = ''%s'' \n', gui_SX.config.CPFEM.msc_module_path);
...
% if ismac || isunix
%     fprintf(fid, 'gb_ind.doit(gb_data, proc_path=r''%s/%s'')\n', gui_SX.config.CPFEM.proc_file_path, gui_SX.GB.Titlegbdata);
% else
%     fprintf(fid, 'gb_ind.doit(gb_data, proc_path=r''%s\\%s'')\n', gui_SX.config.CPFEM.proc_file_path, gui_SX.GB.Titlegbdata);
% end
% fclose(fid);

python4fem_module_path = strrep(gui_SX.config.CPFEM.python4fem_module_path, '\', '\\');
...
proc_path = fullfile(gui_SX.config.CPFEM.proc_file_path, gui_SX.GB.Titlegbdata, '');
proc_path = strrep(proc_path, '\', '\\'); % to escape \r as a carriage return

if strcmp(gui_SX.indenter_type, 'AFM') == 1
    patch2inp(gui_SX.afm_topo_indenter.fvc, proc_path);
end
...
if strcmp(gui_SX.indenter_type, 'AFM') == 1
    patch2inp(gui_SX.afm_topo_indenter.fvc, proc_path_inp);
end
