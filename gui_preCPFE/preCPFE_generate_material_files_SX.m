...
function preCPFE_generate_material_files_SX(CPFEM_code)
...
if strcmp(CPFEM_code, 'GENMAT') == 1
    ...
    fprintf(fid, '1, %s, %s, %s, 0, 0.50, 1\n', ...
        num2str(gui_SX.GB.activeGrain_eul(1)), ...
        num2str(gui_SX.GB.activeGrain_eul(2)), ...
        num2str(gui_SX.GB.activeGrain_eul(3)));
    fclose(fid);
    ...
    genmat.material_dir = fullfile(get_stabix_root, ...
        'gui_preCPFE','genmat','');
    fnameA = fullfile(genmat.material_dir, ...
        genmat.material_fname.(gui_SX.GB.Phase_A));
    copyfile(fnameA, gui_SX.path_config_file)
    fnameB = fullfile(genmat.material_dir, ...
        genmat.material_fname.(gui_SX.GB.Phase_B));
...
elseif strcmp(CPFEM_code, 'DAMASK') == 1
    ...
    scriptname_DAMASK_materialconfig = sprintf(...
        '%s_DAMASK_materialconfig.py', gui_SX.GB.Titlegbdatacompl);
    damask_python_path = strrep(fullfile(...
        gui_SX.config.CPFEM.python4fem_module_path, 'damask'), '\', '\\');
    
    fid = fopen(fullfile(gui_SX.path_config_file, ...
        scriptname_DAMASK_materialconfig), 'w+');
    fprintf(fid, ['# Generated by preCPFE_generate_material_files.m ', ...
        '--- DO NOT EDIT\n']);
    fprintf(fid, 'import sys\n');
    fprintf(fid, 'p=''%s'' \n', ...
        gui_SX.config.CPFEM.python4fem_module_path);
