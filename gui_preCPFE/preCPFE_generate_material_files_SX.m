...
elseif strcmp(CPFEM_code, 'DAMASK') == 1
    ...
    damask_python_path = strrep(fullfile(gui_SX.config.CPFEM.python4fem_module_path, 'damask'), '\', '\\');
    ...
    fprintf(fid, 'damask_mat.mat_config(gb_data, proc_path=r''%s'')\n', fullfile(gui_SX.config.CPFEM.proc_file_path, gui_SX.GB.Titlegbdata));
    fclose(fid);
    % execute the python code that we have just generated
    cmd = sprintf('%s %s', gui_SX.config.CPFEM.python_executable, fullfile(pwd, scriptname_DAMASK_materialconfig));
    commandwindow;
    system(cmd);
