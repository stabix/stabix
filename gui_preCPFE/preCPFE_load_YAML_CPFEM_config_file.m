...
function preCPFE_load_YAML_CPFEM_config_file(...
    YAML_CPFEM_config_file_2_import, varargin)%, interface)
...
if ~isfield(gui.config.CPFEM, 'simulation_code')
    warning_commwin(['Missing CPFEM code definition in your CPFEM YAML ' ...
        'config. file...']);
...
if ~isfield(gui.config.CPFEM, 'fem_solvers')
    warning_commwin(['Missing FEM solvers list in your CPFEM YAML ' ...
        'config. file...']);
...
if ~isfield(gui.config.CPFEM, 'fem_solver_used')
    warning_commwin(['Missing FEM solver used in your CPFEM YAML ' ...
        'config. file...']);
...
if ~isfield(gui.config.CPFEM, 'proc_file_path')
    warning_commwin(['Missing path to store proc. file in your CPFEM ' ...
        'YAML config. file...']);
...
if ~isfield(gui.config.CPFEM, 'python4fem_module_path')
    %warning_commwin(['Missing path for MSC package in your CPFEM YAML ' ...
    %'config. file...']);
...
if ~isfield(gui.config.CPFEM, 'python_executable')
    warning_commwin(['Missing path for Python executable in your ' ...
        'CPFEM YAML config. file...']);
