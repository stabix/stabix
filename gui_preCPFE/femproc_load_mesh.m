...
function femproc_load_mesh(interface)
...
if isequal(YAML_mesh_config_file, 0) || strcmp(YAML_mesh_config_file, '') == 1
    ...
    if interface == 1
        ...
        gui_BX.handles.mesh = femproc_mesh_parameters_BX(mesh_variables);
        guidata(gcf, gui_BX);
        femproc_indentation_setting_BX;
    elseif interface == 2
        gui_SX = guidata(gcf);
        set(gui_SX.handles.pm_mesh_quality, 'Value', 1);
        gui_SX.handles.mesh = femproc_mesh_parameters_SX(mesh_variables);
        guidata(gcf, gui_SX);
