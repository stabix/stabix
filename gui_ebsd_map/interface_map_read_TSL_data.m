...
if gui.flag.newDataFlag == 0
    rdm_TSL_dataset = 0;
    config_map.filename_grain_file_type2              = config_map.default_grain_file_type2;
    config_map.filename_reconstructed_boundaries_file = config_map.default_reconstructed_boundaries_file;
...
elseif gui.flag.newDataFlag == 1
    ...
    if ~isfield(config_map, 'pathname_grain_file_type2') || ~isfield(config_map, 'pathname_reconstructed_boundaries_file') || ~isfield(config_map, 'filename_grain_file_type2') || ~isfield(config_map, 'filename_reconstructed_boundaries_file')
        config_map.filename_grain_file_type2              = config_map.default_grain_file_type2;
        config_map.filename_reconstructed_boundaries_file = config_map.default_reconstructed_boundaries_file;
        config_map.pathname_grain_file_type2              = '';
        config_map.pathname_reconstructed_boundaries_file = '';
...
elseif gui.flag.newDataFlag == 2
    if ismac || isunix
        config_map.pathname_grain_file_type2               = strcat(rdm_TSL_dataset.GF2pathname, '/');
        config_map.pathname_reconstructed_boundaries_file  = strcat(rdm_TSL_dataset.RBpathname, '/');
    else
        config_map.pathname_grain_file_type2               = strcat(rdm_TSL_dataset.GF2pathname, '\');
        config_map.pathname_reconstructed_boundaries_file  = strcat(rdm_TSL_dataset.RBpathname, '\');
    end
    config_map.filename_grain_file_type2              = rdm_TSL_dataset.GF2filename;
    config_map.filename_reconstructed_boundaries_file = rdm_TSL_dataset.RBfilename;
end

%% Read TSL data
try
    GF2_struct = read_oim_grain_file_type2(config_map.filename_grain_file_type2, config_map.pathname_grain_file_type2);
...
if isempty(gui.GF2_struct) %~isfield(dataRB, 'data')
    helpdlg('Empty Reconstructed Boundaries file ! Be careful to have an EBSD map based on an hexagonal grids...');
end
...
try
    RB_struct = read_oim_reconstructed_boundaries_file(config_map.filename_reconstructed_boundaries_file, config_map.pathname_reconstructed_boundaries_file);
    if isstruct(RB_struct)  % if user cancelled file selection, don't overwrite data
        gui.RB_struct = RB_struct;
    end
catch err
    display(err.message);
    config_map.filename_reconstructed_boundaries_file  = 'validation_reconstructed_boundaries.txt';
    config_map.pathname_reconstructed_boundaries_file  = '';
    RB_struct = read_oim_reconstructed_boundaries_file(config_map.filename_reconstructed_boundaries_file, config_map.pathname_reconstructed_boundaries_file);
    if isstruct(RB_struct)  % if user cancelled file selection, don't overwrite data
        gui.RB_struct = RB_struct;
...
set(gui.handles.FileGF2, 'String', fullfile(config_map.filename_grain_file_type2));
set(gui.handles.FileRB, 'String', fullfile(config_map.filename_reconstructed_boundaries_file));
