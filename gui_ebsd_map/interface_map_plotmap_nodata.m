...
if (((maxy*maxx)/size(GF2,1))/(maxy*maxx)) < 0.025
    szFac = ((maxy*maxx)/size(GF2,1))/((maxy+maxx)/2)*5 * ...
        scale_unitcell_val;
else
    szFac = ((maxy*maxx)/size(GF2,1))/((maxy+maxx)/2)*0.5 * ...
        scale_unitcell_val;
end
...
if get(gui.handles.cbgrnum, 'Value') == 1
    for ng = 1:1:max(GF2(:,1))
        gui.handles.h_grain_txt = text(grcen(ng,2), ...
            grcen(ng,3), 3 * szFac, int2str(ng));
        set(gui.handles.h_grain_txt, 'Color', [0,0,0], ...
            'FontWeight', 'bold', ...
...
if get(gui.handles.cbphase, 'Value') == 1 ...
        && get(gui.handles.cbunitcell, 'Value') ~= 1
    for ng = 1:1:max(GF2(:,1))
        if grcen(ng,1) == 0
        else
            if grcen(ng,1) == 1
                gui.handles.h_phase_txt(ng) = ...
                    text(grcen(ng,2)-0.025*maxx, grcen(ng,3)-0.025*maxx, ...
                    int2str(grcen(ng,1)),'Color','g', ...
                    'BackgroundColor', 'none'); %Green
            elseif grcen(ng,1) == 2
                gui.handles.h_phase_txt(ng) = ...
                    text(grcen(ng,2)-0.025*maxy, grcen(ng,3)-0.025*maxy, ...
                    int2str(grcen(ng,1)),'Color','r', ...
...
for ng = 1:1:max(GF2(:,1))
    if grcen(ng,1) == 0
        ...
        eulers_vis_A = ...
            coordinate_system_apply(eulers_A, gui.COORDSYS_eulers);
        if ~isempty(gui.grains(ng).material) ...
                && ~isempty(gui.grains(ng).structure)
            ca_ratio_A   = ...
                listLattParam(gui.grains(ng).material, ...
                gui.grains(ng).structure);
        else
            ca_ratio_A = [];
        end
        ...
        if gui.flag.pmparam2plot_value4Grains == 1 ...
                && get(gui.handles.cbunitcell, 'Value') == 1
            if get(gui.handles.cbphase, 'Value') == 1
                vis_lattice(structure_A, eulers_vis_A, ...
                    slip, shiftxyz, szFac, 0,1, grcen(ng,1));
            else
                vis_lattice(structure_A, eulers_vis_A, ...
                    slip, shiftxyz, szFac, 0,1);
            end
        elseif gui.flag.pmparam2plot_value4Grains == 2 ...
                || gui.flag.pmparam2plot_value4Grains == 3
    ...
    if gui.flag.pmparam2plot_value4Grains == 4
        if get(gui.handles.cbunitcell, 'Value') == 1
            vis_lattice(structure_A, eulers_vis_A, 0, ...
                shiftxyz, szFac, 0,1);
        end
        plot_slip_traces(slip, eulers_vis_A, structure_A, ...
            ca_ratio_A(1), shiftxyz_slip_trace', szFac*2, 0.5);
    end
end
...
if gui.flag.pmparam2plot_value4GB ~= 1
    if get(gui.handles.cbsliptraces, 'Value') == 1
        for gbnum = 1:1:size(RB,1)
            ...
            if gui.grains(gui.GBs(gbnum).grainA).pos_x ...
                    < gui.grains(gui.GBs(gbnum).grainB).pos_x
                shiftxyz_gb_grA = [x_mp + 2*x_mp_delta/size(RB,1); ...
                    y_mp - 2*y_mp_delta/size(RB,1); 0];
                shiftxyz_gb_grB = [x_mp - 2*x_mp_delta/size(RB,1); ...
                    y_mp + 2*y_mp_delta/size(RB,1); 0];
            elseif gui.grains(gui.GBs(gbnum).grainA).pos_x ...
                    > gui.grains(gui.GBs(gbnum).grainB).pos_x
                shiftxyz_gb_grA = [x_mp + 2*x_mp_delta/size(RB,1); ...
                    y_mp + 2*y_mp_delta/size(RB,1); 0];
                shiftxyz_gb_grB = [x_mp - 2*x_mp_delta/size(RB,1); ...
            ...
            elseif gui.flag.pmparam2plot_value4GB == 3 ...
                    || gui.flag.pmparam2plot_value4GB == 7
            ...
            ca_ratio_B = ...
                listLattParam(gui.grains(gui.GBs(gbnum).grainB).material, ...
            ...
            eulers_vis_B = coordinate_system_apply(eulers_B, ...
                gui.COORDSYS_eulers);
            
            plot_slip_traces(slipB, eulers_vis_B, structure_B, ...
                ca_ratio_B(1), shiftxyz_gb_grB, szFac, 0);
        end
    end
end
