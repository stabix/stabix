...
gui.handles.pbFileScanData = set_pushbutton('Import .ang file with MTEX', ...
    [0.02 0.78 0.15 0.03], ['gui = guidata(gcf);' ...
...
gui.handles.pbPlotEBSDMap = set_pushbutton('Plot EBSD Map', ...
    [0.18 0.78 0.11 0.03], ['try MTEX_plotEBSDdata(ebsd);', ...
    'catch err;', ...
    'commandwindow; display(err.message);', ...
    'end']);

gui.handles.pbPlotStatGrain = set_pushbutton('Plot Grain Stat.', ...
    [0.18 0.75 0.11 0.03], ['try MTEX_plotGrainsStat(ebsd);', ...
    'catch err;', ...
    'commandwindow; display(err.message);', ...
    'end']);
...
gui.handles.pbGBplot = set_pushbutton('PLOT BICRYSTAL', ...
    [0.11 0.15 0.1 0.04], 'A_gui_plotGB_Bicrystal(guidata(gcf));');
...
gui.handles.AxisGBmap = axes('Parent', parent,...
